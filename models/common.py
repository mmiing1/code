...
class IEM(nn.Module):
    ...
    def create_prewitt_operator(in_channels):
        ...
        prewitt_x = prewitt_x.repeat(channels, 1, 1, 1)
        prewitt_y = prewitt_y.repeat(channels, 1, 1, 1)

        prewitt_x_filter = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=3, groups=channels,
                                     padding=1, bias=False)
        prewitt_y_filter = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=3, groups=channels,
                                     padding=1, bias=False)
...
class ACblock(nn.Module):
    def __init__(self, in_planes=10, out_planes=10, kernel_att=7, head=4, kernel_conv=3, stride=1, dilation=1):
        ...
        # 初始 1x1 卷积层，后跟 BN 和 ReLU 激活
        self.conv1x1_initial = nn.Conv2d(in_planes, out_planes, kernel_size=1)
        self.bn_initial = nn.BatchNorm2d(out_planes)
        self.relu_initial = nn.ReLU()

        # 分支 1: 1x1 卷积层
        self.branch1_conv1x1 = nn.Conv2d(out_planes, out_planes, kernel_size=1)
        self.branch1_bn = nn.BatchNorm2d(out_planes)
        self.branch1_relu = nn.ReLU()

        # 分支 2: ACmix 模块后接 1x1 卷积层
        self.branch2_acmix = ACmix(in_planes=out_planes, out_planes=out_planes)
        self.branch2_conv1x1 = nn.Conv2d(out_planes, out_planes, kernel_size=1)
        self.branch2_bn = nn.BatchNorm2d(out_planes)
        self.branch2_relu = nn.ReLU()

        # 分支 3: 归一化层
        self.branch3_bn = nn.BatchNorm2d(out_planes)

    def forward(self, x):
        # 初始 1x1 卷积 + BN + ReLU
        out = self.relu_initial(self.bn_initial(self.conv1x1_initial(x)))

        # 分支 1
        branch1_out = self.branch1_relu(self.branch1_bn(self.branch1_conv1x1(out)))

        # 分支 2
        branch2_out = self.branch2_relu(self.branch2_bn(self.branch2_conv1x1(self.branch2_acmix(out))))

        # 分支 3
        branch3_out = self.branch3_bn(out)

        # 将三个分支的输出相加
        final_out = branch1_out + branch2_out + branch3_out
        return final_out
